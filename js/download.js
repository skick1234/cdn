...
const content = game => {
    ...
    game.download.forEach(obj =>
        download.push(
            obj.url
                ? `<a rel="noreferrer" target="_blank" href="${obj.url.replace(
                      /drive\.google\.com.*?\/d\/(.*?)(\/.*?)?$/,
                      "drive.google.com/uc?id=$1&export=download",
                  )}">${obj.name}</a>`
                : obj.name,
        ),
    )
...
${download
    .join("\n")
    .replace(/\(OD\)/g, "(OneDrive)")
    .replace(/\(GD\)/g, "(Google Drive)")}
